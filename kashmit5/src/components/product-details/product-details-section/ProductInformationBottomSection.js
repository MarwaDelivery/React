...
const ProductInformationBottomSection = ({
  ...
  const actionsHandler = () => (
    <Stack direction="row" width="100%" flexWrap={isXSmall && "wrap"} gap={1.2}>
      {productDetailsData?.stock > 0 && isVariationAvailable() || router.query.moduleType === "food" ? (
        <PrimaryButton
      ...
      {!productDetailsData?.isCampaignItem && (
        <>
          {!isInCart(productDetailsData?.id) &&
            (productDetailsData?.stock > 0 || router.query.moduleType === "food") &&
            isVariationAvailable() && (
              <PrimaryButton
                onClick={() => handleVariationAvailability("add")}
                sx={{ width: 200 }}
                disabled={productDetailsData?.stock === 0 && router.query.moduleType !== "food"}
              >
                {t("Add to Cart")}
              </PrimaryButton>
            )}
