...
const DeliverymanInfo = (props) => {
  const { data, configData, t, directions } = props;
  ...
  //     const origin = `${data?.delivery_man?.lat},${data?.delivery_man?.lng}`;
  //     const destination = `${data?.delivery_address?.latitude},${data?.delivery_address?.longitude}`;
  //
    ...
    <CustomStackFullWidth alignItems="center" spacing={1.5}>
      ...
      <CustomStackFullWidth alignItems="center">
        ...
        <CustomTypographyGray sx={{ fontSize: "18px" }}>
          {(directions?.distance?.value / 1000).toFixed(2)}km {t(`${away}`)}
        </CustomTypographyGray>
        <CustomTypographyGray sx={{ fontSize: "18px" }}>
